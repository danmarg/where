...
# Create a temporary directory for all test state and logs
TEST_DIR=$(mktemp -d)
echo "=== E2E Test: Using temporary directory $TEST_DIR ==="

echo "=== E2E Test: Building components ==="
./gradlew :server:installDist :cli:installDist --quiet

echo "=== E2E Test: Starting Server ==="
./server/build/install/server/bin/server > "$TEST_DIR/e2e_server.log" 2>&1 &
SERVER_PID=$!
...
cleanup() {
  ...
  if [ "$TEST_STATUS" == "passed" ]; then
    echo "Test passed, removing temporary directory."
    rm -rf "$TEST_DIR"
  else
    echo "Test failed, keeping $TEST_DIR for debugging."
  fi
...
ALICE_OUT=$(./cli/build/install/cli/bin/cli invite Alice --state "$TEST_DIR/e2e_alice.json" --no-wait)
echo "$ALICE_OUT"
...
./cli/build/install/cli/bin/cli join "$INVITE_URL" Bob --state "$TEST_DIR/e2e_bob.json"
...
ALICE_POLL_OUT=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_alice.json" --once)
echo "$ALICE_POLL_OUT"

echo "=== E2E Test: Alice sends first location ==="
./cli/build/install/cli/bin/cli send 37.7749 -122.4194 --state "$TEST_DIR/e2e_alice.json"

echo "=== E2E Test: Alice sends second location immediately (should be throttled) ==="
ALICE_SEND2=$(./cli/build/install/cli/bin/cli send 37.7750 -122.4195 --state "$TEST_DIR/e2e_alice.json")
echo "$ALICE_SEND2"
...
ALICE_SEND3=$(./cli/build/install/cli/bin/cli send 37.7751 -122.4196 --state "$TEST_DIR/e2e_alice.json" --force)
echo "$ALICE_SEND3"
...
BOB_OUT=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_bob.json" --once)
echo "$BOB_OUT"
...
for i in {1..3}; do
  ...
  ./cli/build/install/cli/bin/cli send "$LAT" "$LNG" --state "$TEST_DIR/e2e_alice.json" --force > /dev/null
  sleep 1
done
echo "✓ Sent multiple messages (epoch rotation may have triggered)"

echo "=== E2E Test: Bob receives ratcheted messages ==="
BOB_RATCHET=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_bob.json" --once)
echo "$BOB_RATCHET"

echo "=== E2E Test: Bidirectional — Bob sends back to Alice ==="
./cli/build/install/cli/bin/cli send 51.5 -0.12 --state "$TEST_DIR/e2e_bob.json" --force > /dev/null
ALICE_RECEIVE=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_alice.json" --once)
echo "$ALICE_RECEIVE"
...
ALICE_RESTART=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_alice.json" --once)
echo "$ALICE_RESTART"
...
CAROL_INVITE=$(./cli/build/install/cli/bin/cli invite Alice --state "$TEST_DIR/e2e_alice.json" --no-wait 2>&1)
...
if [ -z "$CAROL_URL" ]; then
  ...
  echo "  Carol joining..."
  ./cli/build/install/cli/bin/cli join "$CAROL_URL" Carol --state "$TEST_DIR/e2e_carol.json" > /dev/null 2>&1 || true

  echo "  Alice polling to complete key exchange with Carol..."
  ./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_alice.json" --once > /dev/null 2>&1 || true

  echo "  Carol sends location..."
  ./cli/build/install/cli/bin/cli send 48.8566 2.3522 --state "$TEST_DIR/e2e_carol.json" --force > /dev/null 2>&1 || true
...
if [ "$SKIP_MULTI_INVITE" != "1" ]; then
  # Alice creates two invites before anyone joins

  ALICE_OUT1=$(./cli/build/install/cli/bin/cli invite Alice --state "$TEST_DIR/e2e_multi_alice.json" --no-wait)
  ALICE_OUT2=$(./cli/build/install/cli/bin/cli invite Alice --state "$TEST_DIR/e2e_multi_alice.json" --no-wait)
  ...
  ./cli/build/install/cli/bin/cli join "$URL1" Bob --state "$TEST_DIR/e2e_multi_bob.json" > /dev/null
  ./cli/build/install/cli/bin/cli join "$URL2" Charlie --state "$TEST_DIR/e2e_multi_charlie.json" > /dev/null

  # Alice polls once - she should see BOTH of them
  ALICE_POLL_MULTI=$(./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_multi_alice.json" --once)
  ...
  FRIENDS_COUNT=$(./cli/build/install/cli/bin/cli list --state "$TEST_DIR/e2e_multi_alice.json" | grep " (" | grep -c ":")
  if [ "$FRIENDS_COUNT" -eq 2 ]; then
    ...
    ./cli/build/install/cli/bin/cli poll --state "$TEST_DIR/e2e_multi_alice.json" --once > /dev/null
    FRIENDS_COUNT=$(./cli/build/install/cli/bin/cli list --state "$TEST_DIR/e2e_multi_alice.json" | grep " (" | grep -c ":")
    if [ "$FRIENDS_COUNT" -eq 2 ]; then

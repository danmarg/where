...
* **Geofencing:** Upon entering `STILL` the app registers a 200m `Geofence` centered at the current location as a movement tripwire. The OS fires `GEOFENCE_TRANSITION_EXIT` when the user moves >200m away, and that intent itself wakes the service.
* **Low-Power FLP Keepalive (default mode):** On entering `STILL`, the FLP request is downgraded to `Priority.PRIORITY_LOW_POWER` at a 5-minute interval (`HEARTBEAT_INTERVAL_MS`). At this priority FLP serves callbacks from wifi BSSIDs, cell-tower triangulation, and its own cached fix — **it does not power the GNSS chip**. Because the host is a `location` foreground service, these callbacks fire on schedule even while the device is otherwise in Doze. Each callback wakes `pollLoop()`, which sees `isStill == true`, skips the forced GPS fix, and re-sends the cached coordinate as a heartbeat. The doze alarm scheduled in `pollLoop()` and the 15-minute `LocationServiceRestartWorker` remain as belt-and-suspenders backups.
* **Deep-Sleep Mode (`LocationService.deepSleepWhenStationary = true`, opt-in):** Restores the previous behaviour — FLP demoted to `Priority.PRIORITY_PASSIVE` (no GPS unless another app asks), heartbeat cadence governed solely by `setExactAndAllowWhileIdle` + WorkManager. Battery cost drops to near zero, but overnight the cadence collapses to roughly one wake per Doze maintenance window. Use when the friend's overnight visibility doesn't matter.
* **Heartbeat Send:** Every 5 minutes (driven by the FLP callback in keepalive mode, or by the deferred alarm in deep-sleep mode), `pollLoop()` reuses the last cached location to send a heartbeat update to the server. No GPS satellite search is initiated.
* **Geofence Exit / Movement:** When the user moves beyond the 200m geofence, or when the Activity Recognition transitions away from `STILL`, the app removes the geofence, restores `Priority.PRIORITY_HIGH_ACCURACY`, and resumes the active moving rate (10s foreground / 30s background).
...
* **Stationary detection & heartbeat reuse:** When `CLLocationUpdate.stationary` reports stationary for >5 min, we install a 200m `CLCircularRegion` as a movement tripwire (cheap) and stop emitting movement broadcasts. The 5-minute `pollAll()` heartbeat then re-sends the cached coordinate (`bestAvailableLocation`) without forcing a fresh GPS fix, after a `CMMotionActivityManager` sanity check.
* **Resume on movement:** Either the `liveUpdates()` stream re-emits non-stationary deliveries, or the geofence exit fires, or `startMonitoringVisits()` fires — any of these resumes full-rate broadcasting.
* **Deep-sleep mode (`LocationManager.deepSleepWhenStationary = true`, opt-in):** Tears down `CLBackgroundActivitySession` and `liveUpdates()` after 5 minutes stationary. Battery cost drops to near zero, but heartbeat cadence then collapses to whatever iOS chooses to grant via `BGAppRefreshTask` and geofence/visit wakes — typically nothing useful overnight. Use only when the friend's overnight visibility doesn't matter.

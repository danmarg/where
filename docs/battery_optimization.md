...
### 4.1 Android: Activity Recognition, Geofencing, and Low-Power FLP Keepalive
Android does have a guaranteed periodic wake primitive (`setExactAndAllowWhileIdle`), but it is rate-limited to ~once per 9 minutes per app and is further deferred to maintenance windows once the device enters deep Doze (typically after ~1h stationary with the screen off and unplugged). That cadence — potentially hours apart overnight — is too sparse for the 5-minute heartbeat budget in §2.1. To deliver a deterministic heartbeat, we instead lean on the fact that **`FusedLocationProviderClient` callbacks fire even in deep Doze when the host service is a foreground service of type `location`**, and use those callbacks as our wake source.

* **Still Detection:** Using the `ActivityRecognitionClient`, the app detects when the user enters the `STILL` state (stationary for >60s). On this transition the app also immediately sends one `Location` update flagged `stationary = true`, so peers can render "here since HH:mm" instead of an ambiguous "last seen Xh ago" if this device then goes dark — see the shared `GeofencePolicy` note below for why this is sent immediately rather than after a debounce.
* **Geofencing (always-on, not gated on `STILL`):** The app maintains a standing fallback `Geofence` at all times while sharing, centered on the most recent fix, sized and re-centered per the shared `GeofencePolicy` (Kotlin, `shared/src/commonMain/.../GeofencePolicy.kt`): a larger radius while moving (fewer re-registrations needed, since we have live GPS truth anyway) and a tighter radius once `STILL` (position isn't drifting, so any real departure should be caught fast). This is a backstop independent of our own process — the OS watches it even if the app is fully evicted — so it also covers a suspension that happens *while still moving*, which the old `STILL`-only geofence could not. The OS fires `GEOFENCE_TRANSITION_EXIT` on departure, and that intent itself wakes the service, which forces a fresh fix and re-arms immediately (not on any throttle/delay).
* **Low-Power FLP Keepalive:** On entering `STILL`, the FLP request is downgraded to `Priority.PRIORITY_LOW_POWER` at a 5-minute interval (`HEARTBEAT_INTERVAL_MS`). At this priority FLP serves callbacks from wifi BSSIDs, cell-tower triangulation, and its own cached fix — **it does not power the GNSS chip**. Because the host is a `location` foreground service, these callbacks fire on schedule even while the device is otherwise in Doze. Each callback wakes `pollLoop()`, which sees `isStill == true`, skips the forced GPS fix, and re-sends the cached coordinate as a heartbeat. The doze alarm scheduled in `pollLoop()` and the 15-minute `LocationServiceRestartWorker` remain as belt-and-suspenders backups.
* **Heartbeat Send:** Every 5 minutes, driven by the FLP callback, `pollLoop()` reuses the last cached location to send a heartbeat update to the server. No GPS satellite search is initiated.
* **Geofence Exit / Movement:** When the user moves beyond the currently-armed geofence, or when the Activity Recognition transitions away from `STILL`, the app restores `Priority.PRIORITY_HIGH_ACCURACY`, resumes the active moving rate (10s foreground / 30s background), and re-arms the geofence at the new position with the moving radius.

### 4.2 iOS: Persistent Background Activity Session
iOS does not expose any guaranteed periodic wake mechanism to third-party apps. There is no `setExactAndAllowWhileIdle` equivalent: `Timer` is paused while the process is suspended, and `BGAppRefreshTask` cadence is at the system's discretion (frequently zero overnight for idle users). To deliver a reliable 5-minute heartbeat while stationary, we therefore have to keep the process **unsuspended** the whole time.

* **`CLBackgroundActivitySession`:** When the user is sharing with at least one friend, the app holds an active `CLBackgroundActivitySession` and consumes `CLLocationUpdate.liveUpdates()` continuously. This is what keeps the main run loop alive in the background, which in turn keeps the 1Hz `Timer` ticking inside `LocationSyncService`, which fires `pollAll()` every 5 minutes for the heartbeat (`Self.normalPollInterval`).
* **Battery cost rationale:** `liveUpdates()` is documented to throttle radio and GPS usage aggressively while the device is stationary — most stationary deliveries piggyback on existing OS location work rather than activating the GNSS chipset. Empirically the steady-state cost is small, and stationary users (sleeping, at a desk, at home) are very often plugged in.
* **Distance filter:** `CLLocationManager.distanceFilter = 200m` suppresses sub-threshold drift.
* **Stationary detection & immediate signal:** The first time `CLLocationUpdate.stationary` reports stationary, the app immediately sends one `Location` update flagged `stationary = true` (no debounce) so a peer can render "here since HH:mm" before this device might get suspended — an earlier version delayed this behind a live in-process 5-minute `Task.sleep`, which died silently if the app was suspended before it completed, defeating the whole point. The 5-minute `pollAll()` heartbeat then re-sends the cached coordinate (`bestAvailableLocation`) without forcing a fresh GPS fix, after a `CMMotionActivityManager` sanity check.
* **Geofence (always-on, not gated on stationary confirmation):** The app maintains a standing fallback `CLCircularRegion`, sized and re-centered per the shared `GeofencePolicy` (Kotlin, `shared/src/commonMain/.../GeofencePolicy.kt`): larger while moving, tighter once stationary. Because this is watched by CoreLocation independent of our own process, it's the one wake source that survives a suspension mid-motion too, not just after a confirmed stationary period.
* **Resume on movement:** Either the `liveUpdates()` stream re-emits non-stationary deliveries, or the geofence exit fires (which forces a fresh fix and re-arms immediately), or `startMonitoringVisits()` fires — any of these resumes full-rate broadcasting.
* **`BGAppRefreshTask` (fallback only):** A `BGAppRefreshTaskRequest` (`net.af0.where.heartbeat`) is submitted with `earliestBeginDate = +5min`. It is a **best-effort backstop** for cases where the OS suspends the app despite the active session (memory pressure, prolonged idle, OS reboot). It is not the primary heartbeat path and its cadence is not under our control.
* **0 friends / 0 invites:** `LocationManager.updateRegistration()` calls `stopUpdating()` when there are no relationships, which invalidates the session entirely. The `BGAppRefreshTask` is still scheduled unconditionally today (minor: a follow-up could gate this on relationship state per §5).

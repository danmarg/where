...
- **Traffic analysis.** The server sees packet timing, packet sizes, and connection metadata regardless of payload encryption. For a location app, timing is nearly as sensitive as content: update intervals increasing from 30 seconds (moving) to 5 minutes (stationary) reveal movement state; silence reveals offline/stopped status; synchronized update timing across multiple users suggests co-location. A metadata-analyzing server can infer movement patterns from timing alone, independent of content. The 5-minute bucket already has multiple causes bucketed together indistinguishably (stationary, manually paused, and - since the sender throttles sends to a friend whose client hasn't sent anything back in 5 minutes, to avoid wasting radio/battery on updates nobody is reading - now also "friend unresponsive"); adding another cause to an already-ambiguous bucket doesn't hand an observer new distinguishing power. Per-token cadence itself (which friend's traffic slowed) is already fully visible regardless of this feature, via the existing IP-correlation caveat below and via manual per-friend pause, which already produces the identical observable pattern. Mitigations (cover traffic, padding) are discussed in §7.4; they add overhead and battery cost and are not a panacea. This is elevated here as a top-level limitation for threat models that include a compromised or curious server or persistent passive network attacker.

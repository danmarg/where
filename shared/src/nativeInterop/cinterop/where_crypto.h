...
/* Forward-declare Security framework constants */
...
/* Forward declarations: CCCryptorGCMOneshot* */
...
int where_aesgcm_encrypt(const uint8_t key[32], const uint8_t nonce[12],
    ...
    int st = CCCryptorGCMOneshotEncrypt(
        0, key, 32, nonce, 12, aad, aadlen, pt, ptlen,
        out_ct_tag, out_ct_tag + ptlen, &tagLen);
    return (st == 0) ? 0 : -1;
}
...
int where_aesgcm_decrypt(const uint8_t key[32], const uint8_t nonce[12],
    ...
    int st = CCCryptorGCMOneshotDecrypt(
        0, key, 32, nonce, 12, aad, aadlen,
        ct_tag, ctLen, out_pt, ct_tag + ctLen, 16);
    return (st == 0) ? 0 : -1;
}

...
class LocationHandshakeTest {
    ...
    @Test
    fun testHandshakeMatchingWithMultipleInvites() =
        runTest {
    ...
    @Test
    fun testLocationUpdatesFlowImmediatelyAfterHandshake() =
        runTest {
            val aliceDriver = createTestSqlDriver()
            val aliceManager = testE2eeManager(aliceDriver)
            val aliceClient = LocationClient("http://localhost", aliceManager, mailbox)
     ...
     */
    @Test
    fun testPollDoesNotConsumeInvite() =
        runTest {
            val aliceDriver = createTestSqlDriver()
            val aliceManager = testE2eeManager(aliceDriver)
            val aliceClient = LocationClient("http://localhost", aliceManager, mailbox)
     ...
     */
    @Test
    fun testAliceSendsLocationToBobAfterHandshake() =
        runTest {
            val aliceDriver = createTestSqlDriver()
            val aliceManager = testE2eeManager(aliceDriver)
            val aliceClient = LocationClient("http://localhost", aliceManager, mailbox)

...
class CatchUpTest {
    ...
    fun testCatchUpLargeBacklog() =
        runTest {
            ...
            // Alice's recvToken is Bob's sendToken, but here we want to put messages where
            // Alice will poll. Alice polls HER recvToken; Bob sends to Alice's recvToken.
            val bobToken = bobSession.recvToken.toHex()
    ...
    fun testForceAckBreaksLoop() =
        runTest {
            ...
            for (i in 1 until MAX_SILENT_DROP_RETRIES) {
                aliceClient.poll()
                assertNotNull(
                    mailbox.mailboxes[currentAliceRecvToken]?.find { it.msgId == "bad-msg" },
                    "Should not have ACKed yet at retry $i",
                )
            }

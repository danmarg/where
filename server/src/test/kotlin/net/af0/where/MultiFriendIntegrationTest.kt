...
class MultiFriendIntegrationTest {
    ...
    class KtorTestMailboxClient(private val client: HttpClient) : MailboxClient {
        override suspend fun post(baseUrl: String, token: String, payload: MailboxPayload) {
            val resp = client.post("/inbox/$token") {
    ...
    @Test
    fun `central Alice receives updates from Bob and Charlie in the same poll`() = testApplication {
    ...
    @Test
    fun `poll updates friend location persistently in store`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `central Alice receiving from Bob does not interfere with Charlie receiving from Alice`() = testApplication {
        application { module(ServerState()) }

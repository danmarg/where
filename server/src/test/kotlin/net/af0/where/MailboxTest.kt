...
class MailboxTest {
    ...
    @Test
    fun `POST inbox returns 204`() = testApplication {
        application { module(ServerState()) }
        val response = client.post("/inbox/aabbccddeeff0011") {
    ...
    @Test
    fun `POST inbox with empty body returns 400`() = testApplication {
        application { module(ServerState()) }
        val response = client.post("/inbox/aabbccddeeff0011") {
            contentType(ContentType.Application.Json)
            setBody("")
    ...
    @Test
    fun `POST inbox with non-JSON body returns 400`() = testApplication {
        application { module(ServerState()) }
        val response = client.post("/inbox/sometoken") {
            contentType(ContentType.Application.Json)
            setBody("not json")
        }
        assertEquals(HttpStatusCode.BadRequest, response.status)
    ...
    @Test
    fun `GET inbox for unknown token returns empty array`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `GET inbox drains messages posted to that token`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `GET inbox is destructive - second GET returns empty`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `GET inbox for different tokens are independent`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `GET inbox response is identical for unknown and empty tokens`() = testApplication {
        application { module(ServerState()) }

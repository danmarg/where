...
class E2eeIntegrationTest {
    ...
    private fun fp(ikPub: ByteArray, sigIkPub: ByteArray): ByteArray =
        MessageDigest.getInstance("SHA-256").digest(ikPub + sigIkPub)

    private fun ByteArray.toHex(): String = joinToString("") { "%02x".format(it) }

    private fun generateIdentity() = IdentityKeys(
    ...
    fun `key exchange produces identical routing token and chain key on both sides`() {
        ...
        assertContentEquals(
            aliceSession.routingToken, bobSession.routingToken,
            "routing tokens must match",
        )
        assertContentEquals(
            aliceSession.sendChainKey, bobSession.sendChainKey,
    ...
    fun `key exchange QR signature verification rejects tampered QR`() {
        ...
        val tampered = QrPayload(
            ikPub = qr.ikPub, ekPub = qr.ekPub,
            sigPub = tamperedSigPub,
            suggestedName = qr.suggestedName, fingerprint = qr.fingerprint, sig = qr.sig,
        )
    ...
    @Test
    fun `full flow - key exchange then encrypt location through mailbox then decrypt`() = testApplication {
        ...
        val payload: MailboxPayload = EncryptedLocationPayload(
            epoch = newAliceState.epoch,
            seq = newAliceState.sendSeq.toString(),
            ct = ct,
        )
        val postResp = client.post("/inbox/$token") {
        ...
        val decryptResult = Session.decryptLocation(
            bobSession, received.ct, received.seqAsLong(), aliceFp, bobFp,
        )
    ...
    @Test
    fun `multiple sequential location updates are decryptable in order`() = testApplication {
        application { module(ServerState()) }
        ...
        for (loc in locations) {
            ...
            client.post("/inbox/$token") {
                contentType(ContentType.Application.Json)
                setBody(json.encodeToString<MailboxPayload>(
        ...
        for ((i, el) in arr.withIndex()) {
            ...
            val (newBobState, decrypted) = Session.decryptLocation(
    ...
    @Test
    fun `ciphertext tampered in transit fails decryption`() = testApplication {
        application { module(ServerState()) }
    ...
    @Test
    fun `routing token isolation - wrong token mailbox is empty`() = testApplication {
        application { module(ServerState()) }

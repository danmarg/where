...
Build signed standardGms (universal) + standardFdroid (per-ABI split) release APKs for the current versionName and publish them as assets on a GitHub Release tagged v<versionName>. The standardFdroid APKs are the signed reference binaries F-Droid's Reproducible Builds feature diffs its own build against (see the per-Build `binary:` fields in metadata/net.af0.where.yml upstream — one per ABI). Signing stays local — nothing here touches GitHub Actions or CI secrets. Run this after committing the versionName/versionCode bump (e.g. from deploy/deploy_full) so HEAD is the exact commit you want the tag to point at. Requires signing env vars and an authenticated `gh` CLI.
